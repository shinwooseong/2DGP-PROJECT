...
class Animator:
    ...
    Supports per-state images (horizontal) and single-sheet modes:
      ...
      - image_list: list of individual image paths for animation frames
    Computes per-frame bboxes using PIL when available.
    """
    def __init__(self, folder, frames_map, frame_time, layout='horizontal', single_image_path=None, single_frame_height=None, image_list=None):
        self.folder = folder
        ...
        if image_list:
            try:
                ...
                for img_path in image_list:
                    ...
                    self._image_list_loaded.append(img)
                # frames_map에서 첫 번째 state의 프레임 수를 image_list 길이로 설정
                first_state = list(frames_map.keys())[0]
                self.frames_map[first_state] = len(image_list)
                ...
                if DEBUG_MONSTER:
                    print(f"Animator: failed to load image_list: {e}")
                self._image_list = None
                self._image_list_loaded = []
    ...
    def draw(self, x, y, scale=1.0):
        ...
        if self._image_list is not None and self._image_list_loaded:
            frames = len(self._image_list_loaded)
            if frames > 0:
                idx = int(self.frame) % frames
                img = self._image_list_loaded[idx]
                try:
                    dw = int(img.w * scale)
                    dh = int(img.h * scale)
                    img.draw(x, y, dw, dh)
